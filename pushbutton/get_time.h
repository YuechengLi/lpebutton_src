...
typedef struct{
	// To handle the case when more than two sets of sensor data are obtained within one second, 
	// "pre_time" is used to store the time of previous set of data and then is compared with current
	// time. "inx" is used to distinguish two file names.
	char pre_time[10];
	int inx;
} timecomp;
