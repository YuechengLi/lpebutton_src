...
#include "opus.h"
/*The frame size is hardcoded for this sample code 
...
#define BUF_SIZE NUM_FRAMES_PER_SEC*RECORDING_SEC	\
*FRAME_SIZE*(CHANNELS*Format/8) 
...
volatile unsigned char new_audio_ppflag = 0;
volatile unsigned char new_audio_data=0;
...
const char *imu_DEV= "/dev/iio:device1";
...
volatile char low_power_mode = 1;
volatile char image_encryption_en = 0;

//pingpong buffer for sensor data
volatile unsigned char flag_newdata=0;
volatile unsigned char flag_sensorbuffer=0;
#define LEN_SENSOR	256//256
MPU9250_sensor MPU9250_DATA[2*LEN_SENSOR];


//pingpong buffer for mcu sensor data
volatile unsigned char flag_newdata_mcu=0;
volatile unsigned char flag_sensorbuffer_mcu=0;
...
volatile unsigned char imu_file_open=0;
volatile unsigned char mcu_file_open=0;
volatile unsigned char audio_file_open=0;

pthread_mutex_t lock;
pthread_t imuThread,imusaveThread, msp430imageThread,	\
	msp430Thread, msp430saveThread,	\
	camThread,gpsThread,dmicThread, dmicThread_save,	\
	poweronoffThread, lowbatteryThread;

volatile option_struct  global_options;

volatile unsigned char BT_connected=0;
unsigned char *pImage;
unsigned char *pImage_a;
unsigned char *pImage_b;
volatile unsigned char pingpong_bt = 0;

volatile unsigned char IMG_CNT = 0;
volatile unsigned char BT_IMG_EN = 0;

volatile unsigned int numFrames = 0;
unsigned int numTails=0;
volatile unsigned char image4bt_ready = 0;
volatile unsigned char image_trans_ing = 0;
...
volatile int motion_poweroff = 0;
...
{
	//decive function, such as video input
...

	{
		...
		{	 
			...
			if(jpg_size<24000)
			{
				...
				if((IMG_CNT>cnt_trigger)&(((IMG_CNT-cnt_trigger)%(cnt_trigger/40)) == 0))
				{
					moto_ctrl(1);
					//IMG_CNT = 0;
				}
	...
	{
		{
			...
			//motion detect
			if(global_options.AUTO_OFF_EN)
			{
				if(k == 0)
				{
					accX = pMPU9250_DATA->ACC_fl[0];
					accY = pMPU9250_DATA->ACC_fl[1];
					accZ = pMPU9250_DATA->ACC_fl[2];
				}
				if((k>0)&(k%30 == 0))
				{
					motion_acc +=  abs((int)(1000*(pMPU9250_DATA->ACC_fl[0]-accX))) 
							+ abs((int)(1000*(pMPU9250_DATA->ACC_fl[1]-accY))) 
							+ abs((int)(1000*(pMPU9250_DATA->ACC_fl[2]-accZ)));
					printf("%d,%d,%f,%f\n", cnt_motiondetect, motion_acc,pMPU9250_DATA->ACC_fl[0], accX);

					if(cnt_motiondetect == 200)
					{		
						//poweroff		
						if(motion_acc < 2000)
						{
							...
							if(atoi(usb)==0)//usb is not connected, stop lp_ebutton*/
								motion_poweroff = 1;//power_off();
						}
...
//			if(cnt_sample_imu==(LEN_SENSOR-1))
			...
			if((global_options.MCU_MODE==2)&(BT_connected==1)&(image_trans_ing==0)) 
			{
				...
				//usleep(5000+5000*global_options.DMIC_ON);
...
void *dmicFunction_save()
{
	...
	short *audio_buffer_read_short;
	short audio_data_amp;
		...
		if(new_audio_data)
		{
			...
			if(audio_file_open)
			{
				...
				for (k=0;k<NUM_FRAMES_PER_SEC*RECORDING_SEC;k++)
				{
					...
					audio_buffer_read_short = (short *)(audio_buffer_read+2*k*CHANNELS*FRAME_SIZE);
					for (i=0;i<CHANNELS*FRAME_SIZE;i++)
					{
						in[i] = (opus_int16)(20*(audio_buffer_read_short[i]));//(pcm_bytes[2*i+1]<<8)|pcm_bytes[2*i]));
					}
				...
				//raw data
				//fwrite(audio_buffer_read, 1, BUF_SIZE, fp);
...
void *LowBattery_detection()
{
	...
	while (1)
	{
		...
		if((atoi(nlbo)==0)|(motion_poweroff == 1)){//check battery, if low, shutdown
...
*/
	...
	//use imu to detect motion for auto power_off
	if(global_options.AUTO_OFF_EN == 1)
...
/*		if(start_timer(3000, &camFunction_timer)){
	...
	if (global_options.DMIC_ON)
	{
		...
		else
		{
			...
			pthread_attr_setinheritsched(&thread_attr_dmic, PTHREAD_EXPLICIT_SCHED); 
			pthread_attr_setschedpolicy(&thread_attr_dmic,SCHED_RR);
			schedule_param_dmic.sched_priority = 1;
			pthread_attr_setschedparam(&thread_attr_dmic, &schedule_param_dmic); 
			pthread_create( &dmicThread, &thread_attr_dmic, dmicFunction, NULL);
			//pthread_create( &dmicThread, NULL, dmicFunction, NULL);
