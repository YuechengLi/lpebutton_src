			...
			if(global_options.MCU_MODE==2){
				...
				//if(!image_trans_ing)
				{
					...
					if(BT_connected==1){
						...
						numFrames = (jpg_size/Size_frame);
						numTails = jpg_size%Size_frame;

						image4bt_ready = 1;

						while(!image_trans_ing);//wait BT to start transfer
						while(image_trans_ing);//wait untill BT finished sending
					}
				}
...
void *msp430_image_trans_only()
{
	...
	while(1)
	{
		...
		if((BT_connected==1)&(image4bt_ready==1)){//connected
			...
			//sending data to msp430, 7Bytes/sample
			//buffer_bt[0] = 0x48;//indicating valid image data

			//for test
			for (i=0;i<2000;i++) {
				//generate test data
				for(k=0;k<Size_frame;k++){buffer_bt[k]=test_data;}
			...
			/*for (i=0;i<numFrames;i++) {
				...
				MSP430_write_bt(pImage, Size_frame);
				printf(".");
			...
			printf("  totally %d frames were sent!\n",numFrames+1);
...
/*		if(start_timer(3000, &camFunction_timer)){
	...
	//if (global_options.MCU_BARO_ON|global_options.MCU_MOTION_ON|global_options.MCU_LIS_ON|global_options.MCU_OPTIC_ON)
	{
		...
		MSP430_init_config(4000000, msp430_cfg_data );
	...
	{
		pImage = (unsigned char *)malloc(200*1024);//100KB buffer for image
