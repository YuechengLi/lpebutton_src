...
int process_image(unsigned char *addr, int length, char *imFile)
{
...

	{
		...
		{	 
			...
			//BT trans
			if((global_options.MCU_MODE==2)&(BT_IMG_EN>=2)&(image_trans_ing==0)){
...
//			if(cnt_sample_imu==(LEN_SENSOR-1))
			...
			{;}*/
			usleep(1000);//give 90sps averagely
...
void *msp430_image_trans_only()
{
	...
	while(1)
	{
		...
		if((BT_connected==1)&(image4bt_ready==1)){//connected
			...
			for(k=0;k<970000;k++)//delay, important
				{;}
	...
	/*		for (i=0;i<numFrames-1;i++) {
				//generate test data
				for(k=0;k<Size_frame;k++){buffer_bt[k+numHeader]=test_data+k;}
				//for(k=0;k<Size_frame;k++){buffer_bt[k]=test_data+k;}
				//printf("test_data: 0x%x ",test_data);
				//test_data++;
				//if(test_data==0x5B)test_data=0x41;
				...
				pImage += Size_frame;
				for(k=0;k<850000;k++)//for(k=0;k<600000;k++)
				{;}
...
*/
			...
			for (i=0;i<numFrames-1;i++) {
				...
				printf(".");if(i%100==0) printf(".. %d sent!\n", i);
...
void *msp430_image_trans_only()
{
	...
	while(1)
	{
		...
		if((BT_connected==1)&(image4bt_ready==1)){//connected
			...
			buffer_bt[7] = numFrames/256;
			MSP430_write_bt(buffer_bt, Size_frame);//+numHeader);
			for(k=0;k<700000;k++)//delay, important
				{;}
*/
			/*do{
...
/*
			...
			for (i=0;i<numFrames-1;i++) {
				//generate test data
				for(k=numHeader;k<Size_frame;k++){buffer_bt[k]=test_data+k;}
				//for(k=0;k<Size_frame;k++){buffer_bt[k]=test_data+k;}
				//printf("test_data: 0x%x ",test_data);
				//test_data++;
				//if(test_data==0x5B)test_data=0x41;
	
				MSP430_write_bt(buffer_bt, Size_frame);//test data

				printf(".");
				if(i%100==0) printf(".. %d sent!\n", i);

				pImage += (Size_frame-2);
				for(k=0;k<700000;k++)//for(k=0;k<600000;k++)
				{;}
...
/*
			//sending tail
			printf("...sending tail...");
			buffer_bt[Size_frame-2] = 0x37;
			buffer_bt[Size_frame-1] = 0x73;
			for (i=numHeader;i<numTails;i++) 
				buffer_bt[i] = pImage[i];
			MSP430_write_bt(buffer_bt, Size_frame);
			printf("totally %d frames were sent! numTails=%d\n",numFrames, numTails);
			/////////////////////////////////////////////////////////////////////////////////////
			//clear BT status buffer
			//BT_connected = MSP430_read_bt_status();//numFrames);
...
void *camFunction()
{
	...
	{		
			...
			if(low_power_mode==1)
			{
				...
				{
					image4bt_ready = 1;
					sleep(numFrames/7);//sleep long time when BT transfer is used
	...
	while (1)
	{
		if(new_audio_data)
		{
			...
			if(!audio_file_open)
			{
				...
				audio_file_tail = (atoi(time_struct.time_min))/5;//new file every 5 minutes
			
				//sprintf(auFile,"/boot/uboot/Data/%d%s",k,".hex");
				//sprintf(auFile,"%s%s%c%s%s",hourDir,deviceID,'_',yrMonDayHrMinSec,".hex");
				sprintf(auFile,"%s%s%s%02d%s",hourDir,deviceID,"_record_",audio_file_tail,".hex");
				printf(auFile);
				printf("\n");
			...
			if(audio_file_open)
			{
				...
				for (k=0;k<NUM_FRAMES_PER_SEC*RECORDING_SEC;k++)
				{
					...
					pcm_bytes = audio_buffer_read+k*CHANNELS*FRAME_SIZE;
...
/*		if(start_timer(3000, &camFunction_timer)){
	...
	//if (global_options.MCU_BARO_ON|global_options.MCU_MOTION_ON|global_options.MCU_LIS_ON|global_options.MCU_OPTIC_ON)
	{
		...
		MSP430_init_config((1000000+2000000*(2-global_options.MCU_MODE)), msp430_cfg_data );
	...
	else if(global_options.MCU_MODE==2)
	{
		...
		//pImage_b = (unsigned char *)malloc(200*1024);//200KB buffer for image
