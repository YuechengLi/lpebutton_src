...
unsigned char gpiod14_read()
{
	...
	gpioD14_value[0] = 0;
	gpioD14_fd = fopen("/sys/class/gpio/pioE24/value", "rb");
	fread(gpioD14_value, sizeof(char), 1, gpioD14_fd);
	//printf("gpioD14: %d\n",atoi(gpioD14_value));
	fclose(gpioD14_fd);

	return atoi(gpioD14_value);
}
...
/*			for (i=0;i<numFrames-1;i++) {
				...
				//{;}
				for(k=0;k<600000;k++)//delay, important
				{;}
				do{BT_send_ready = gpiod14_read();}while(BT_send_ready==0);
...
void *camFunction()
{
	...
	{		
			...
			if(low_power_mode==1)
			{
				...
				{
					image4bt_ready = 1;
					sleep(numFrames/9);///8);//sleep long time when BT transfer is used
/*					
					for(i=0;i<100;i++){;}//delay
