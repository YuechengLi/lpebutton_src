...
void create_HourPath(char *hourDir)
{
	...
	strcat(moDayDir,FOLDER_NAME_t);
	//printf("folder is %s\n",FOLDER_NAME);
	//printf("moDayDir is %s\n",moDayDir); 
	strcpy(sys_cmd_t,"mkdir -p ");
	strcat(sys_cmd_t,moDayDir);
	system(sys_cmd_t);

	
	sprintf(moDayDir_t,"%s%c",moDayDir,'/'); //moDayDir: sdcard/month.day/
	...
	while(1)
	{
		if(flag_newdata)//new data is availible
		{
			if(!imu_file_open){
				...
				sprintf(imuFile,"%s%s%02d%s",hourDir,"imu_sensordate_",imu_file_tail,file_format);//".txt");
	...
	while(1)
	{
		if(flag_newdata_mcu)//new data is availible
		{
			if(!mcu_file_open){
				...
				sprintf(hourDir,"%s%d%c",moDayDir_t,psaveMCU_DATA->hour,'/');//
				mcu_file_tail = (psaveMCU_DATA->min)/file_interval;//new file every 10 minutes
				sprintf(mcuFile,"%s%s%02d%s",hourDir,"mcu_sensordate_",mcu_file_tail,file_format);//".txt");
...
int main(int argc, char *argv[])
{
	...
	//pthread_create( &lowbatteryThread, NULL, LowBattery_detection, NULL);
